    ...
    int monitor_sockets(int udp_watch_sockets, int tcp_watch_sockets)
    {
        int udp_data_size= 0, tcp_data_size = 0;
        struct sockaddr udp_saddr, tcp_saddr;
        int saddr_size = sizeof(udp_saddr);
        while(true)
        {
            tcp_data_size = recvfrom(tcp_watch_sockets, &tcp_buffer, IP_MAXPACKET, &tcp_saddr, &saddr_size);
